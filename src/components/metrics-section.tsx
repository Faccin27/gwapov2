...
const testimonials: Testimonial[] = [
	{
		quote:
			"A mudança para a Gwapo trouxe um valor infinito que ainda estou descobrindo hoje.",
	...
	{
		quote:
			"A equipe da Gwapo superou as expectativas. A atenção aos detalhes e a abordagem moderna são inigualáveis.",
		author: "David Lee",
...
export default function MetricsSection() {
	...
	const animateCounters = () => {
		...
		const animate = (currentTime: number) => {
			...
			setAnimatedCounts(
				metrics.map((metric) => Math.floor(metric.numericValue * easeOutQuart)),
			);
		...
		>
			...
			<div className="relative h-[400px] md:h-[500px] overflow-hidden">
				<div
					...
					style={{
						background:
							"linear-gradient(to top right, rgba(244, 63, 94, 0.2) 70%, transparent 90%)",
				...
				<div className="absolute inset-0 flex items-center justify-center">
					<div
						ref={metricsRef}
						className="relative w-full h-full max-w-screen-xl mx-auto px-4 sm:px-6 md:px-8"
					>
						{isDesktop ? (
							...
							<div className="relative w-full h-full">
								{metrics.map((metric, index) => (
									...
									>
										...
										<p className="font-medium text-gray-800 text-lg md:text-xl">
											{metric.description}
										</p>
									</div>
								))}
							...
							<div className="flex flex-col justify-center h-full space-y-8 px-4">
								{metrics.map((metric, index) => (
									<div
										key={index}
										className="metric-item flex items-center gap-4"
									>
