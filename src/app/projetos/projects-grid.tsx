...
const projectTypes = ["Todos", ...PROJECT_TYPES];
...
export function ProjectsGrid({ projects }: { projects: Project[] }) {
  return (
    <Suspense fallback={null}>
      <ProjectsGridContent projects={projects} />
    </Suspense>
  );
}

function ProjectsGridContent({ projects }: { projects: Project[] }) {
  const searchParams = useSearchParams();
  const tipoParam = searchParams.get("tipo");
  const initialType = projectTypes.includes(tipoParam ?? "") ? (tipoParam as string) : "Todos";
                ...
                <div className="flex flex-wrap gap-2">
                  <span
                    className={cn(
                      ...
                      typeColors[project.type] ?? "bg-white/5 text-muted-foreground border-white/10"
                    )}
                  >
                    {project.type}
                  </span>
